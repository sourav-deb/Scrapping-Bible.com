...
# Chapters are hardcoded and sub-chapters are automated.
...
def bibleChapter(a):
    ...
    try:
        url = "https://my.bible.com/bible/406/2SA."+GENESIS+".ERV?parallel=2671"
        driver.get(url)
        ...
        if len(newEngVerse) == len(newNepVerse):
            ...
            df = df.to_excel('./Bible/2 SAMUEL/2 SAMUEL '+GENESIS+'.xlsx', index = True, index_label='Serial')
            print("EXCEL CREATED...")
            # -----------------
        else:
...
while (i<51):
    GENESIS = i
    GENESIS = str(GENESIS)
    print("Chapter "+GENESIS)
    bibleChapter(GENESIS)
    i+=1
